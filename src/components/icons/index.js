...
export const Back = ({size, onPress, style, color, isClose}) => {
  const defaultSize = size || totalSize(3);
  return (
    <Icon
    ...
    <TouchableOpacity
      ...
      {svgIcon ? (
        ...
        <Text
          isRegular
          isMediumFont
          style={[{color: textColor || colors.appColor1}, textStyle]}>
          {text}
        </Text>
      ) : (
        <Icon
...
export const Svg = ({svg, onPress, size}) => {
  ...
  return (
    <Pressable onPress={onPress} disabled={!onPress}>
      <Svg height={defaultSize} width={defaultSize} />
    </Pressable>
  );
};
